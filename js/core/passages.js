...
//
// LENGTH, and why some passages are still short.
//
// These ran 310 to 617 words, median 400, against roughly 750 in the Quest
// booklet and 688 to 1008 in GL's published English papers. They now run
// 379 to 793, median 692, with 21 of 40 at or above GL's floor.
//
// They were lengthened by splicing source text AROUND the existing paragraphs
// rather than re-extracting, so every stored paragraph survives byte for byte
// and all 480 questions stayed valid by construction.
//
// The first attempt at this was thrown away. It extended each passage into
// whatever happened to sit next to it, and a read of the 16,000 added words
// found 24 of 40 unusable. It had pulled in Gutenberg front matter (chapter
// lists, transcriber credits, plate lists, a German bibliography), and worse,
// content nobody had chosen: a racial description of an enslaved child in Tom
// Sawyer, a sustained passage on Satan in Crusoe, a man being crushed in
// Shackleton, a drunk and violent household in the Pauper.
//
// The lesson is that the original forty extracts were CHOSEN and their
// neighbours were not. So the second attempt screened at the point of choice:
// front matter stripped first, then each extension picked against the same
// rules used to select the originals, rejecting a paragraph and everything
// past it on racial or colonial language, religious instruction, cruelty,
// vivid death, alcohol shown approvingly, or prose too dense to read.
//
// Three passages take NO extension, because the text around them is unusable
// in every direction:
//
//   tom        the next paragraph describes a child by race, then a fist fight
//   robinhood  ale as a prize for a boy, then a deer and a man killed
//   lobo       scalp bounties, then 250 sheep killed for sport
//
// Black Beauty is short for the same reason: its one safe-looking extension
// turned out to contain the master's pet name for the black colt, which is a
// slur now whatever it meant in 1877.
//
// Short and clean beats long and unsafe. That is the trade, made on purpose.
//
